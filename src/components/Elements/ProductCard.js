...
export const ProductCard = ({product}) => {
    const {best_seller,rating,id,name,price,overview,poster}=product;
          ...
          <div className="p-5">
              ...
              <p className="flex justify-between items-center">
                 ...
                 { !inCart&&<button onClick={()=>addToCart(product)} className={`inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 ${product.in_stock ? "": "cursor-not-allowed"}`}  disabled={ product.in_stock ? "" : "disabled"}>Add To Cart <i className="ml-1 bi bi-plus-lg"></i></button>}
                  {inCart && <button onClick={()=>removeFromCart(product)}className="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800">Remove Item <i className="ml-1 bi bi-trash3"  disabled={ product.in_stock ? "" : "disabled"}></i></button> }
              </p>
          </div>
      </div>
    )
